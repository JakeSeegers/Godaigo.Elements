...
### Duplicate scroll bug (e.g. Psychic in hand AND common area simultaneously) – **not fixed, root cause not confirmed**
- User report from a 2026-08-13 playtest (4-player multiplayer, 2 human + 2 bot): the Psychic scroll (`VOID_SCROLL_1`) appeared in a player's hand while it was already sitting in the common area. The attached action log (`js/ActionLog` export) didn't cover the moment it happened, so this could not be reproduced/root-caused directly — only investigated by code reading.
- `SpellSystem.validateScrollState()` (`js/game-core.js`) already detects exactly this state (hand+commonArea or active+commonArea holding the same scroll name) and logs a `[Scroll state] ⚠️ DUPLICATE` warning — so if it happens again, the browser console will show it immediately, including which two locations disagree.
- **Confirmed self-healing gap:** the auto-correction only fires `if (errorsFound && isMultiplayer)` **and** `myPlayerIndex !== 0` (`js/game-core.js` around `validateScrollState()`) — i.e. only non-host clients ever request a `scroll-state-sync-request`. If the desync originates on the **host's own client**, nothing ever corrects it, since the host is always treated as authoritative. Worth adding a host-side self-check (e.g. periodic `getScrollStateSnapshot()` broadcast, or the host running its own correction pass against itself) rather than assuming the host can't drift.
- Traced the two most plausible mechanical entry points for a hand/common-area overlap without confirming either is the actual cause:
  - Psychic's own disposition (`countered-original` handling in `js/multiplayer-state.js`) removes the scroll from the counter-caster's `active` set before calling `discardToCommonArea` — but only `if (counterScrolls.active.has(scrollName))`. If some client's local state never saw the hand→active move that `playerResponds()`/`handleRemoteResponse()` (`js/scrolls/response-window.js`) are supposed to apply before this runs, that guard silently no-ops and the scroll would still be sitting in that player's `hand` when `discardToCommonArea` puts it in the common area on other clients — a plausible race, not yet proven.
  - `drawScrollFromDeck()` (`js/game-ui.js`, the dev "browse deck" tool) draws straight from `scrollDecks[element]` by index with no check against `commonArea`/other players' hands — low risk in normal play since decks and the common area are supposed to be mutually exclusive, but worth a defensive check if this tool is ever exposed outside dev use.
- Next step if it recurs: capture the browser console at the moment it's noticed (the `[Scroll state] ⚠️ DUPLICATE` warning will name both locations and the player indices) — that pins down which code path actually put it in both places, rather than guessing further.
...
- **Undo let you keep a scroll's cost-free effect (2026-08-13 playtest report)** – `js/game-core.js`'s `executeSpell()`. User suspected undo was used unfairly late-game "after I activated a scroll." Confirmed: casting a scroll (`executeSpell()`) spent AP and could touch other players' state, but never cleared the one-step undo (`lastMove` / `window.lastScrollAction`) left over from whatever move/stone-place/stone-break/scroll-move happened right before the cast — so clicking Undo afterward silently reversed that PRIOR action (refunding AP/a stone) while the scroll's real effects stayed applied, a free double-dip. Fixed by clearing both the instant AP is spent for the cast, mirroring the existing clear on tile reveal (also correctly irreversible).
- **Scroll panel Activate/Move buttons vanishing until the panel was closed and reopened (2026-08-13 playtest report)** – `js/lobby.js`'s `turn-change` and `turn-sync` broadcast handlers. Root cause: `js/scroll-panels.js`'s `_buildCard()` gates the whole action-button row on a `canModify` check computed from `activePlayerIndex === myPlayerIndex` **at render time only** — but neither handler that updates `activePlayerIndex` (normal turn advance, or the periodic desync-correction `turn-sync` listener) ever told `ScrollPanelSystem` to re-render. A panel left open across a turn change (or a desync correction) kept showing cards built while it was someone else's turn, buttons and all, until manually toggled closed/open forced a fresh `_buildCard()` pass. Fixed by calling `ScrollPanelSystem.refresh()` in both places.
- **Take Flight (Wind IV) – targeted player's own pawn couldn't be dragged, stalling the spell until cancelled (2026-08-13 playtest report, root cause found on follow-up)** – `js/game-core.js`'s `placePlayer()`. User clarified after the initial description reword: "the other player was never granted the actual ability to move to another tile, which stalled the spell and caused the other to have to cancel." Root cause: a player's pawn `<g>` element is recreated (with fresh event listeners) via one of three code paths — (1) initial pawn creation, (2) `placePlayer(x, y)` re-rendering the **active** player's own pawn after their own move, (3) `movePlayerVisually()` re-rendering **another** player's pawn from a broadcast. Paths 1 and 3 both check `window.takeFlightState` in their `mousedown`/`touchstart` handlers and allow the drag when this pawn is the current Take Flight target, even though it isn't that player's turn — but path 2 only checked `currentIdx === activePlayerIndex`, with no such bypass. Since a player's own pawn is normally last (re-)rendered via path 2, right after their own most recent move, its listener was always missing the target-drag bypass by the time anyone else's turn came around and targeted them with Take Flight — clicking their pawn did nothing (the click then got silently swallowed further up by the board's generic `selectionMode` click-consumption in `handleSelectionModeClick()`, `js/game-core.js`, which has no branch for `take-flight-drag` and just eats the event). Fixed by adding the same `takeFlightState` bypass to path 2's `mousedown` and `touchstart` handlers, mirroring paths 1 and 3.
  Also reworded the description per an earlier request in this same session: "Target player must move to an unoccupied hex on an elemental or catacomb tile of their choice. That tile must be occupied by another player." (`js/scrolls/scroll-definitions.js`, text-only).
...
### To verify (playtest reports, not yet confirmed as bugs)
- [ ] **"Didn't get fire stones after ending turn on a shrine"** – single report from the 2026-08-13 playtest; the reporting user (host) was themselves skeptical it's a real bug rather than a misremembered turn/shrine. Not investigated yet — no repro steps or log entry to anchor it (the action log's `endTurn`/`placeStone` entries don't distinguish "stood on a shrine at end of turn" from other reasons stones changed). Needs a repro before spending time on it: which shrine, whose stones were expected, what turn.
...
*Last updated: 2026-08-13 playtest glitch triage — fixed undo-after-scroll-cast exploit and stale scroll-panel buttons after a turn change/desync correction, reworded Take Flight's description again, logged the Psychic duplicate-scroll report (root cause not confirmed) and the shrine-fire-stones report (unverified) for follow-up.*
