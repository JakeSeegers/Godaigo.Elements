...
- **Take Flight (Wind IV) – description reworded again (2026-08-13 playtest report)** – `js/scrolls/scroll-definitions.js`. User again found Take Flight "didn't work properly" in play and asked for clearer wording; new description: "Target player must move to an unoccupied hex on an elemental or catacomb tile of their choice. That tile must be occupied by another player." Text-only change — the underlying destination logic (`getValidTakeFlightDestinations()` in `js/scrolls/effects/scroll-effects.js`, excludes player tiles and face-down tiles, requires another player currently on the tile) was not re-verified against this specific playtest's actual behavior since no repro steps or log entries for a Take Flight cast were available; if it still misbehaves, capture what happened (who was targeted, what destinations were offered/rejected) next time.
