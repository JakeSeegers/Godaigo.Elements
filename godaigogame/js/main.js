// ========================================
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.addEventListener('scroll-resolved', (event) => {
                ...
                if (entry && entry.scrollData && entry.result === 'resolved') {
                    // Apply the scroll effects for scrolls that resolved from the response stack
                    // (not the original scroll - that's handled separately)
                    if (!entry.isOriginal && typeof spellSystem !== 'undefined') {
                        console.log(`Applying resolved scroll effects: ${entry.scrollData.name}`);
                        spellSystem.applyScrollEffects(
                            entry.scrollData.name,
                            entry.scrollData.definition || spellSystem.patterns[entry.scrollData.name]
                        );
                    }
                }
            });
        ...
        class SpellSystem {
            ...
            getPlayerScrolls(forDisplay = false) {
                ...
                if (!this.playerScrolls[displayIndex]) {
                    this.playerScrolls[displayIndex] = {
                        hand: new Set(),      // Scrolls in hand (private, max 4)
                        active: new Set(),    // Scrolls in active area (name+element visible to opponents, max 2)
                        activated: new Set()  // Elements used for win condition
                    };
                }
            ...
            // Move scroll from active area back to hand (0 AP cost)
            moveToHand(scrollName) {
                const scrolls = this.getPlayerScrolls(false); // Use active player for game logic

                // Check hand size limit
                if (scrolls.hand.size >= this.MAX_HAND_SIZE) {
                    updateStatus(`Hand full! Max ${this.MAX_HAND_SIZE} scrolls. Discard one first.`);
                    return false;
                }
                ...
                if (scrolls.active.has(scrollName)) {
                    ...
                    console.log(`📜 Moved ${scrollName} back to hand`);
                    this.updateScrollCount();
            ...
            onTileRevealed(shrineType) {
                ...
                if (scrolls.hand.size >= this.MAX_HAND_SIZE) {
                    // Check if active area has room - prompt to move a scroll there
                    if (scrolls.active.size < this.MAX_ACTIVE_SIZE) {
                        this.showMoveToActivePrompt(selected, scrollInfo, shrineType);
                        return scrollInfo;
                    } else {
                        // Both hand and active are full - must discard
                        this.showDiscardPrompt(selected, scrollInfo, shrineType);
                        return scrollInfo;
                    }
                }
            ...
            // Show a prompt to move a scroll to active area when hand is full but active has room
            showMoveToActivePrompt(newScrollName, newScrollInfo, shrineType) {
                ...
                Object.assign(popup.style, {
                    ...
                    zIndex: '1002', minWidth: '400px', maxWidth: '550px',
                    maxHeight: '80vh', overflowY: 'auto', color: 'white'
                });

                const title = document.createElement('h2');
                title.textContent = '📜 Hand Full! Move a Scroll to Active Area';
                ...
                subtitle.innerHTML = `You found "<strong>${newScrollInfo.name}</strong>" but your hand is full (${this.MAX_HAND_SIZE}/${this.MAX_HAND_SIZE}).<br>Choose a scroll to move to your Active Area (${this.activeScrolls.size}/${this.MAX_ACTIVE_SIZE}):`;
                ...
                // Create a card for each scroll in hand + the new scroll
                const allScrollOptions = [...scrolls.hand, newScrollName];
                ...
                allScrollOptions.forEach(scrollName => {
                    ...
                    Object.assign(card.style, {
                        backgroundColor: isNew ? '#1a3a1a' : '#34495e',
                        border: isNew ? '2px solid #27ae60' : '1px solid #555',
                        borderRadius: '8px',
                        padding: '12px',
                        marginBottom: '10px',
                        cursor: 'pointer',
                    ...
                    card.onmouseenter = () => card.style.backgroundColor = '#f39c12';
                    card.onmouseleave = () => card.style.backgroundColor = isNew ? '#1a3a1a' : '#34495e';
                    ...
                    nameSpan.textContent = (isNew ? '✨ NEW: ' : '') + (pattern ? pattern.name : scrollName);
                    nameSpan.style.fontWeight = 'bold';
                    nameSpan.style.color = elementColor;
                    header.appendChild(nameSpan);

                    const actionBtn = document.createElement('span');
                    actionBtn.textContent = '⚡ To Active';
                    actionBtn.style.color = '#f39c12';
                    ...
                    card.onclick = () => {
                        if (scrollName === newScrollName) {
                            // Move new scroll directly to active, add nothing to hand
                            ...
                            updateStatus(`Moved new scroll "${newScrollInfo.name}" directly to Active Area`);
                        } else {
                            // Move existing scroll to active, add new scroll to hand
                            scrolls.hand.delete(scrollName);
                            scrolls.active.add(scrollName);
                            ...
                            updateStatus(`Moved "${self.patterns[scrollName]?.name || scrollName}" to Active Area, added "${newScrollInfo.name}" to hand`);
                        }
            ...
            // Show a prompt to discard a scroll when both hand and active are full
            showDiscardPrompt(newScrollName, newScrollInfo, shrineType) {
                const popup = document.createElement('div');
                Object.assign(popup.style, {
                    position: 'fixed', left: '50%', top: '50%',
                    transform: 'translate(-50%, -50%)',
                ...
                const title = document.createElement('h2');
                title.textContent = '📜 All Slots Full! Choose a Scroll to Discard';
                title.style.textAlign = 'center';
                ...
                const subtitle = document.createElement('p');
                subtitle.innerHTML = `You found "<strong>${newScrollInfo.name}</strong>" but your hand (${this.MAX_HAND_SIZE}/${this.MAX_HAND_SIZE}) and active area (${this.MAX_ACTIVE_SIZE}/${this.MAX_ACTIVE_SIZE}) are both full.<br>Choose a scroll to discard:`;
                subtitle.style.textAlign = 'center';
                subtitle.style.color = '#bdc3c7';
                popup.appendChild(subtitle);
                ...
                // Create a card for each scroll in hand + active + the new scroll
                const allScrollOptions = [...scrolls.hand, ...scrolls.active, newScrollName];
                ...
                allScrollOptions.forEach(scrollName => {
                    ...
                    card.onmouseenter = () => card.style.backgroundColor = '#e74c3c';
                    card.onmouseleave = () => card.style.backgroundColor = isNew ? '#1a3a1a' : (isInActive ? '#3a2a1a' : '#34495e');
                    ...
                    card.onclick = () => {
                        if (scrollName === newScrollName) {
                            // Discard the new scroll - send to common area
                            self.discardToCommonArea(newScrollName);
                            updateStatus(`Discarded the new scroll: ${newScrollInfo.name} (sent to Common Area)`);
                        } else {
                            // Discard existing scroll (from hand or active) and add new one to hand
                            if (isInActive) {
                                scrolls.active.delete(scrollName);
                            } else {
                                scrolls.hand.delete(scrollName);
                            ...
                            updateStatus(`Discarded "${self.patterns[scrollName]?.name || scrollName}" to Common Area, kept "${newScrollInfo.name}"`);
                        }
            ...
            executeSpell({name, spell, fromCommonArea = false}) {
                // Spend AP first
                spendAP(this.SPELL_AP_COST);

                const scrollData = { name, spell, fromCommonArea, casterIndex: activePlayerIndex };

                // Check if response window system is available
                if (this.responseWindow && this.responseWindow.canAnyPlayerRespond()) {
                    // Open response window - effects will be applied after it closes
                    ...
                    this.applyScrollEffects(name, spell);
                }
            }
            ...
            handleResponseWindowComplete(originalScrollData, result) {
                if (result.skipped) {
                    ...
                    this.applyScrollEffects(originalScrollData.name, originalScrollData.spell);
                    return;
                }
            ...
            showInventory() {
                ...
                const createScrollCard = (scrollName, scroll, element, location) => {
                    ...
                    // Move button (only show if it's my turn in multiplayer, or always in single player)
                    const canMove = !isMultiplayer || (myPlayerIndex === activePlayerIndex);
                    if (canMove) {
                        const moveBtn = document.createElement('button');
                        moveBtn.textContent = location === 'hand' ? '➡ To Active' : '⬅ To Hand';
                        Object.assign(moveBtn.style, {
                            padding: '5px 10px', fontSize: '12px', cursor: 'pointer',
                            backgroundColor: location === 'hand' ? '#27ae60' : '#e74c3c',
                ...
                if (commonAreaScrolls.length === 0) {
                    ...
                    elementTypes.forEach(element => {
                        if (this.commonArea[element]) {
                            ...
                            const symbol = element === 'catacomb' ? '⚰️' : STONE_TYPES[element].symbol;
                ...
                infoText.innerHTML = '<strong>Tip:</strong> Move scrolls to Active Area (0 AP) to prepare for activation. Scrolls in your Active Area OR the Common Area can be cast. Discarded scrolls go to Common Area (max 1 per element).';
 ...
 // Default 2 minutes in milliseconds (set by host)
        ...
        function updateVoidAP() {
            ...
            console.log(`🔄 updateVoidAP: ${oldVoidAP} → ${voidAP} (playerPool.void=${playerPool.void})`);
        ...
        let tileMoveMode = false; // When true, placed tiles can be dragged
        ...
        function placeTile(x, y, rotation = 0, flipped = false, shrineType = null, isPlayerTile = false, skipMultiplayerLogic = false, forcedTileId = null) {
            ...
            tileGroup.addEventListener('mousedown', (e) => {
                ...
                if (e.button === 0 && !tileHasStones(tileId) && !isPanning && !isDraggingStone && !isDraggingPlayer) {
...
function clearPlayerPath() {
        ...
        function updatePlayerPath(x, y) {
            ...
            if (moveCheck.canMove) {
                ...
                if (totalCost + moveCheck.cost <= getTotalAP()) {
                    ...
                    if (shouldLog) {
                        console.log(`✅ Added to path! Total cost now: ${totalCost + moveCheck.cost}`);
                    }
                } else if (shouldLog) {
                    console.log(`⚠️ Cannot extend path to (${best.x.toFixed(1)}, ${best.y.toFixed(1)}): Insufficient AP (need ${totalCost + moveCheck.cost}, have ${getTotalAP()})`);
                }
            } else if (shouldLog) {
        ...
        function calculateTapMoveCost(startPos, endPos) {
            ...
            while (queue.length > 0) {
                ...
                for (const neighbor of neighbors) {
                    ...
                    const newCost = cost + (moveCheck.cost || 1);
        ...
        function updateStoneCount(type) {
            ...
            if (newSourceEl) newSourceEl.textContent = sourcePool[type];
        ...
        function setupStoneDragFromCard(card, element) {
            card.addEventListener('mousedown', (e) => {
                ...
                // Trigger the same drag behavior as the old stone deck items
                const oldDeck = document.getElementById(element + '-deck');
                if (oldDeck) {
                    // Simulate the drag start on the old element
                    startStoneDrag(e, element);
                }
            });
        }

        function startStoneDrag(e, type) {
            ...
            for (let i = 0; i < totalPlayers; i++) {
                ...
                if (activeSize > 0) {
                    ...
                    scrollData.active.forEach(scrollName => {
                        ...
                        const elementColor = element === 'catacomb' ? '#9b59b6' : STONE_TYPES[element]?.color || '#666';
                        const elementSymbol = element === 'catacomb' ? '⚰️' : STONE_TYPES[element]?.symbol || '?';
        ...
        function handleBoardTouchEnd(e) {
            ...
            console.log(`📱 Tap-to-move check: isTap=${isTap}, isDraggingTile=${isDraggingTile}, isDraggingStone=${isDraggingStone}, isDraggingPlayer=${isDraggingPlayer}, ghostTile=${!!ghostTile}, ghostStone=${!!ghostStone}, playerPosition=${!!playerPosition}, isMyTurn=${isMyTurn()}`);
            if (isTap && !isDraggingTile && !isDraggingStone && !isDraggingPlayer && !ghostTile && !ghostStone && playerPosition && isMyTurn()) {
                const world = screenToWorld(screenX, screenY);
...
document.getElementById('undo-move').onclick = function() {
        ...
        // Tile Move Mode toggle
        document.getElementById('tile-move-toggle').onclick = function() {
            tileMoveMode = !tileMoveMode;
            const btn = this;
            if (tileMoveMode) {
                btn.textContent = '🔒 Lock Tiles';
                btn.style.background = '#f44336';
                updateStatus('🔓 Tile move mode ON - tap tiles to move them');
            } else {
                btn.textContent = '🔓 Move Tiles';
                btn.style.background = '';
                updateStatus('🔒 Tile move mode OFF - tiles are locked');
            }
        };
        ...
        function findShrineAtPosition(x, y) {
            ...
            for (const tile of placedTiles) {
                ...
                if (dist < 5) {
                    console.log(`🏛️ findShrineAtPosition(${x.toFixed(1)}, ${y.toFixed(1)}): Found ${tile.shrineType} shrine at tile ${tile.id}`);
                    return tile;
                }
            }
            console.log(`🏛️ findShrineAtPosition(${x.toFixed(1)}, ${y.toFixed(1)}): No shrine found`);
            return null;
        }
        ...
        function updateCatacombIndicators() {
            ...
            if (otherCatacombs.length === 0) {
                updateStatus('⚰ Standing on catacomb shrine, but no valid destinations! (catacombs must be revealed and have no stone on center)');
                return;
            }
            ...
            otherCatacombs.forEach(shrine => {
                ...
                indicator.addEventListener('click', (e) => {
                    ...
                    if (hasStoneNow) {
                        updateStatus('⚰ Cannot teleport there - a stone is blocking the catacomb!');
            ...
            updateStatus(`⚰ Standing on catacomb shrine. Click on another revealed catacomb (glowing circles) to teleport there (free).`);
        }
        ...
        function setupGameBroadcast() {
            ...
            gameChannel.on('broadcast', { event: 'scroll-collected' }, ({ payload }) => {
                ...
                updateOpponentPanel(); // Update opponent panel when scroll collected
            });
            ...
            gameChannel.on('broadcast', { event: 'scroll-move' }, ({ payload }) => {
                ...
                updateOpponentPanel(); // Update opponent panel when scroll moves
            });
            ...
            gameChannel.on('broadcast', { event: 'scroll-discard' }, ({ payload }) => {
                ...
                if (spellSystem.playerScrolls[playerIndex]) {
                    ...
                    // Don't add to common area here - that's handled by the common-area-update event
                    console.log(`📜 ${getPlayerColorName(playerIndex)} discarded ${scrollName}`);
                    updateOpponentPanel(); // Update opponent panel when scroll discarded
                }
            });
