        ...
        class SpellSystem {
            ...
            applyScrollStateSnapshot(snapshot) {
                ...
                if (snapshot.players) {
                    for (let i = 0; i < snapshot.players.length; i++) {
                        ...
                        // Sync activated (win condition tracking)
                        const newActivated = new Set(snap.activated || []);
                        if (!this.setsEqual(p.activated, newActivated)) {
                            console.log(`🔄 Player ${i} activated corrected:`, Array.from(p.activated), '→', Array.from(newActivated));
            ...
            onSelectionEffectComplete(scrollName, effectName, spell) {
                ...
                if (typeof updatePlayerElementSymbols === 'function') {
                    updatePlayerElementSymbols(this.activePlayerIndex);
                }
