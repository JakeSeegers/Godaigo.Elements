...
const ScrollEffects = {
    ...
    processReflectPending(playerIndex) {
        ...
        if (myReflects.length === 0) return null;
        ...
        myReflects.forEach((pending, index) => {
            ...
            if (result.success && this.spellSystem && !waterActivated) {
    ...
    processPsychicPending(playerIndex) {
        ...
        myPsychics.forEach((pending, index) => {
            ...
            const result = this.execute(scrollName, playerIndex, {
                spell: definition,
                scrollName,
                skipActivationTracking: true  // Only void counts toward win condition
            });

            // Restore original activePlayerIndex
            ...
            if (result.success && this.spellSystem && !voidActivated) {
                this.spellSystem.ensurePlayerScrollsStructure(playerIndex);
                const activated = this.spellSystem.playerScrolls[playerIndex].activated;
                activated.add('void');
                voidActivated = true;
