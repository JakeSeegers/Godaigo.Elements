            ...
            if (handOver || activeOver) {
                spellSystem.showEndTurnOverflowModal(function doEndTurn() {
                    ...
                    if (playerPositions.length > 1) {
                        ...
                        if (isMultiplayer) {
                            // Process reflects (now returns array or null)
                            if (Array.isArray(reflectResult)) {
                                reflectResult.forEach(result => {
                                    if (result.triggered && result.definition) {
                            ...
                            // Process psychics (now returns array or null)
                            if (Array.isArray(psychicResult)) {
                                psychicResult.forEach(result => {
                                    if (result.triggered && result.definition) {
                                        const def = result.definition;
