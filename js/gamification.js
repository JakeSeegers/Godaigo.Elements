...
//
// XP rewards:
//   element_activated : +15 XP (up to 5× per game)
...
window.gami = (function () {
    ...
    const api = {
         ...
         * +15 XP per activation (up to 5 per game win).
         */
        async onElementActivated(element, allActivated) {
            if (!_userId) return;
            const xp = 15;
            try {
                const { error } = await supabase.rpc('update_user_xp', {
                    p_user_id:     _userId,
                    p_xp_points:   xp,
                    p_description: `Element activated: ${element}`
                });
                if (error) { console.error('[gami] element xp error:', error); return; }

                _logActivity('element_activated', xp, 0, `Activated ${element}`, { element });
                ...
                api.notify(`+${xp} XP — ${label} activated!`, xp, 'xp');
            } catch (err) {
                console.error('[gami] onElementActivated error:', err);
            }
        },
