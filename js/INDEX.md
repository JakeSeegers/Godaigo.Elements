...
| `bot-arena.js` | ~380 | `window.BotArena`: Stage 3a self-play arena — `run(weightsA, weightsB, nGames, seed)` plays local hot-seat 2-player bot-vs-bot games (seeded decks, alternating sides, muted sound/XP/win-modal), `evolve()` = Gaussian weight evolution, `spectate(n)` = watchable 2–5-bot match with normal visuals + auto action-log download (cheat panel "🤖 Bot match" row, `stop()` to end early). First measurements: hybrid search beat greedy 12-3-5 → hybrid is now the default brain. |
