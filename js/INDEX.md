...
| `bot-effects.js` | ~250 | `window.BotEffects.driveSelection()`: Stage 2.5, in progress — drives scroll-effect selection choices (tile-flip, scorched-earth, tile-swap, Create, Scholar's Insight so far) instead of `waitForQuiescence()` just cancelling them. Calls the game's own `selectionMode.handleXClick()`/modal buttons directly, never reimplements validity. Full inventory + remaining scrolls: `docs/bot-roadmap.md` § STAGE 2.5. |
