...
| `game-ui.js` | ~3200 | **UI layer**: drag-drop for pawns/stones/tiles, HUD updates, panel collapse, scroll deck UI, opponent panel, common area. `initBotTrainingPanel()` builds the player-facing "🧬 Bot Training" full-screen modal (`#bot-training-overlay`, triggered by clicking the Profile modal's header 5× in 3s): a **Method** row (`Evolve (GA)` / `Hill Climb`) picks the trainer; Players (2/3/4/5/**All**)/Speed/Repeat controls with explainer tooltips — "All" is the generalist mode: `runWeightTraining()` sends `nPlayers:'all'` so training spans mixed 2–5-player arenas and the end gate becomes `BotArena.confirmAcrossSizes()` (champion vs a field of baselines at every size) instead of the fixed-count evolve + 2-player run() gate. Selecting Hill Climb forces/disables Players at 2 (`BotArena.hillClimb()` has no nPlayers concept) and routes Start Training through `runHillClimbTraining()` instead — explicitly fetches the ONLINE champion from `bot_champion_weights` to anchor to (aborts rather than silently using defaults if that fails), climbs, then runs its own separate confirm series against that same baseline before applying/submitting; shares the confirm-gate/apply/submit pattern with `runWeightTraining()` but is a fully separate function, and skips the population roster (no population concept in hillclimb). Wires `hillClimb()`'s `onChallenger`/`onGame` callbacks through to `showTrainingPopup()` so the corner popup updates on every game (not just once per round) and names exactly what's happening — current challenger number, current game number, and which real in-game color (Purple/Yellow, from `colorRankOrder`) each side is playing as THIS game, recomputed from the game's parity every report since `_playSeries` alternates sides each game. Popup header reuses the same `.panel-header`/`.panel-title`/`.hud-toggle-btn` classes the opponent Players panel uses, not one-off inline styles. Popup body is structured (scenario/phase/matchup/progress-bar/history/summary elements, not one text blob): a real progress-bar fill, a compact round-history chip strip (gold=promoted, dark=held, dashed=in-progress, tooltip per chip) as the "how have rounds gone" summary, and small color-swatch spans (real hex, not just names) next to Challenger/Champion labels. No emoji anywhere in this player-facing panel (the separate hidden dev cheat panel elsewhere in this file still uses emoji on its own buttons — deliberately out of scope, not player-facing). A Population roster (clickable rows keyed by `bot-arena.js`'s member `id`s, fitness bar, lineage label — "elite (surviving)" / "bred #X×#Y" / "mutated #X" / "seed") and a Generations log, both fed by the shared `handleGeneration(gen, total, fitnessArr, members)` callback wired into `runWeightTraining`'s/`evolve()`'s `onGeneration`. Clicking a roster row opens a weight-diagram detail view (`weightBar()`) grouped by `WEIGHT_CATEGORIES`, comparing that member's weights against `BotSystem.DEFAULT_WEIGHTS`. A separate, outer-scope persistent popup (`ensureTrainingPopup()`/`showTrainingPopup()`/`hideTrainingPopup()`, `#bot-training-status-popup`, fixed bottom-right) shows scenario + progress info for whichever run is active and survives the modal being closed/reopened (unlike the modal's own contents, which are rebuilt fresh each open) — has "End Early → Test Now" (`BotArena.endEarly()`, hidden once the confirming phase starts) and "Stop" (`BotArena.stop()`) buttons, plus an "⤢" button (`window._openBotTrainingPanel`) to reopen the full modal. Below the roster: "Deploy this bot" (publishes `window.BotSystem.WEIGHTS`, or a captured bot picked from a Source select, into `deployed_bots` — see gamification-ui.js's Stable tab for the once-per-source limit) and "Challenge other bots". No "Breed from champion files" flow anymore — removed per explicit user request; `evolve()`'s own internal crossover (bot-arena.js) is unrelated and unaffected. `state.sourceBotId`/`sourceBotNickname` (set only when opened via Stable's Train button) route a successful run's result into that specific bot's `deployed_bots` row instead of the ambient weights/community table — see `runWeightTraining()`/`runHillClimbTraining()`. |
...
| `gamification-ui.js` | ~800 | Profile modal (8 tabs: Stats, Colours, Emojis, Badges, Shop, Stable, Leaderboard, Settings). Shop/Stable are the Bot Tycoon economy tabs (docs/bot-tycoon-proposal.md) — Shop sells Capture Stones, Stable lists your deployed bots (retire/reactivate, Train) and captured bots (redeploy under a new name). Train applies that specific bot's weights (`window.BotArena.applyWeights`), signals `window._botTrainingSource = {id, nickname}`, and opens the Bot Training panel (game-ui.js) — a successful run writes results back into that bot's own `deployed_bots` row instead of just the ambient live weights. `gami_openPanelOnTab(tab)` opens the modal directly on a given tab (auth-bar Shop/Stable buttons in index.html), toggling closed on a second click same as Profile. |
